...
exports.addclockEntrry = async (req, res) => {
    try {
        ...
        if (type === 'clock_in') {
            ...
            return res.status(201).json({ status: 0, message: 'Clock-in successful', clockin });
        }
...
exports.AllProjectList = async (req, res) => {
    try {
        const projectList = await db.Project.findAll({ where: { company_id: req.user.company_id } });
...
exports.addclockImgaes = async (req, res) => {
    try {
        ...
        const project = await db.Project.findByPk(project_id);
        if (!project) return res.status(400).json({ status: 0, message: 'Project Not Found' });
        const fileValidation = await validateFiles(clock_images, ["jpg", "jpeg", "png", "webp"], 15 * 1024 * 1024);
...
//         // 2. Date range for current month
//         const startOfMonth = moment().startOf('month').toDate();
//         const endOfMonth = moment().endOf('month').toDate();

//         // 3. Fetch work sessions for the current month
//         const sessions = await db.Clock_entry.findAll({
//             where: {
//                 worker_id: req.user.id,
//                 clock_in_time: { [Op.gte]: startOfMonth },
//                 clock_out: { [Op.lte]: endOfMonth },
//             },
//             attributes: ['clock_in_time', 'clock_out']
//         });

//         // 4. Calculate total worked minutes
//         let totalMinutes = 0;
//         sessions.forEach(session => {
//             if (session.clock_in_time && session.clock_out) {
//                 const start = moment(session.clock_in_time);
//                 const end = moment(session.clock_out);
...
//         const workedFormatted = `${workedHours}:${workedMinutes.toString().padStart(2, '0')}`;

//         // 5. Fetch monthly expected hours from branch
//         const branch = await db.Company.findOne({
//             where: { id: user.company_id },
//             imclude:[
//                 {
//                     model: db.Branch,
//                     as: 'industry' ,
//                     attributes: ['monthly_hours'],
//                 }
//             ]
//         });

//         const totalMonthlyHours = branch?.industry?.monthly_hours || 0;

//         // 6. Calculate percentage
//         const percentComplete = totalMonthlyHours > 0
//             ? Math.min(((workedHours + workedMinutes / 60) / totalMonthlyHours) * 100, 100).toFixed(1)
//             : '0.0';
...
//                 user,
//                 work_summary: {
//                     worked_time: workedFormatted,
//                     total_target_hours: totalMonthlyHours,
...
//         // 2. Get clock entries for the month
...
//         // 4. Get monthly target from branch (industry)
...
exports.homeScreenCount = async (req, res) => {
    try {
        ...
        // 3. Sum all durations (in seconds)
        let totalSeconds = 0;
        entries.forEach(entry => {
        ...
        //         start_date: { [Op.lte]: moment().format('YYYY-MM-DD') },  // Start date before or equal to today
        //         end_date: { [Op.lte]: moment().format('YYYY-MM-DD') },  // End date before or equal to today
        //     },
        //     include: [
        //         {
        //             model: db.Absences,
        ...
        //     const startDate = moment(request.start_date);
        //     const endDate = moment(request.end_date);
        //     const diffDays = endDate.diff(startDate, 'days') + 1;  // Including both start and end date
        ...
        usedRequests.forEach(request => {
            const startDate = moment(request.start_date);
            const endDate = request.end_date ? moment(request.end_date) : startDate; // For single day, end date is same as start date
            const diffDays = endDate.diff(startDate, 'days') + 1;  // Including both start and end date
            usedDays += diffDays;
        });
        ...
        const plannedRequests = await db.absence_request.findAll({
            where: {
                worker_id: req.user.id,
                start_date: { [Op.gt]: moment().format('YYYY-MM-DD') },  // Start date after today
            },
            include: [
                {
                    model: db.Absences,
